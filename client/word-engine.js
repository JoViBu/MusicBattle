...
function wordAt(board,placements,r,c,dr,dc){
  const placed=new Map(placements.map(p=>[key(p.r,p.c),p.letter]));
  const letter=(rr,cc)=>placed.get(key(rr,cc))||board[rr]?.[cc]||null;
  while(letter(r-dr,c-dc)){r-=dr;c-=dc}
  let word='',cells=[];
  while(r>=0&&c>=0&&r<SIZE&&c<SIZE&&letter(r,c)){word+=letter(r,c);cells.push({r,c,isNew:placed.has(key(r,c))});r+=dr;c+=dc}
  return {word,cells};
}
function scoreWord(wordData){let base=0,mult=1;for(let i=0;i<wordData.cells.length;i++){const cell=wordData.cells[i],letter=wordData.word[i];let points=POINTS[letter]||1;if(cell.isNew){const premium=premiumAt(cell.r,cell.c);if(premium==='dl')points*=2;if(premium==='tl')points*=3;if(premium==='dw'||premium==='center')mult*=2;if(premium==='tw')mult*=3}base+=points}return base*mult}
function validateMove(board,placements,dictionary){
  ...
  const clean=placements.map(p=>({r:Number(p.r),c:Number(p.c),letter:normalize(p.letter)}));
  ...
  const invalid=unique.find(item=>!wordsSet.has(normalize(item.word)));
  if(invalid)return {ok:false,message:`“${invalid.word.toLocaleLowerCase('ca')}” no és al diccionari.`};
...
function rackCanSupply(word,board,r,c,dr,dc,rack){const counts={};rack.forEach(letter=>counts[letter]=(counts[letter]||0)+1);const placements=[];for(let i=0;i<word.length;i++){const rr=r+dr*i,cc=c+dc*i;if(rr<0||cc<0||rr>=SIZE||cc>=SIZE)return null;const fixed=board[rr][cc];if(fixed&&fixed!==word[i])return null;if(!fixed){if(!counts[word[i]])return null;counts[word[i]]--;placements.push({r:rr,c:cc,letter:word[i]})}}return placements.length?placements:null}
